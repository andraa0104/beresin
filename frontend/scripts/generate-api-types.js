...
const BACKEND_DOCS_JSON = process.env.API_DOCS_URL || 'http://localhost:3000/api/docs-json';
const OUTPUT_FILE = path.resolve(__dirname, '../src/types/api.generated.ts');

async function generateTypes() {
  console.log(`📡 Fetching OpenAPI spec from: ${BACKEND_DOCS_JSON}`);
  try {
    const res = await fetch(BACKEND_DOCS_JSON);
    if (!res.ok) {
      throw new Error(`Failed to fetch OpenAPI JSON: ${res.status} ${res.statusText}`);
    }
    const spec = await res.json();
    console.log(`✓ Fetched OpenAPI: ${spec.info?.title} (v${spec.info?.version})`);
    console.log(`✓ Total endpoints found: ${Object.keys(spec.paths || {}).length}`);

    // Generate TypeScript type definitions
    let output = `/**
 * AUTO-GENERATED FILE. DO NOT MODIFY MANUALLY.
 * Generated from OpenAPI Spec: ${spec.info?.title} (${spec.info?.version})
...
`;
    ...
    console.log(`✓ TypeScript API types successfully generated at: ${OUTPUT_FILE}`);
  } catch (err) {
